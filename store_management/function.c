...
void searchByName(struct category* _category, int size)	//truyen` tham tri. vao _category vi` ham` isFounded can truyen con tro?
{
	...
	for (int i = 0;i < size;i++) {
		...
		//	break;
		//}
		if (strstr(_category[i].categoryName, name) != NULL) {		//tim` tuong doi', chi? can` nhap. 1 phan` ten
			flag = 0;
			if (_flag) {
...
void saveCategoryData(struct category* _category, int* size)
{
	...
	case 1: //mo? file va` lay' du~ lieu. tu` file
	{
		...
		fread(_category, sizeof(struct category), *size, file);
		fclose(file);
...
void searchProductByName(struct product* _product, int size)
{
	...
	for (int i = 0;i < size;i++) {
		...
		//	break;
		//}
		if (strstr(_product[i].productName, name) != NULL) {	//tim` kiem tuong doi'
			flag = 0;
			if (first) {
				first = 0;
				printf("Founded! Here is information about product\n");
...
void saveProductData(struct product* _product, int* size)
{
	int choice;
	printf("1. Load data from file\t2. Save data to file\t0. Back\nEnter your choice: ");
	scanf_s("%d", &choice);
	...
	case 1: //mo? file va` lay' du~ lieu. tu` file
	{
		...
		fread(size, sizeof(int), 1, file);
		fread(_product, sizeof(struct product), *size, file);
		fclose(file);
		printf("Load category from file success!!\n");
		break;
	}
...
void manageProduct(int* command, struct product* _productList, int* size, struct category* _categoryList, int* length)
{
	while (1)
	{
		...
		if (*command >= 1 && *command <= 9) {
			...
			case 2:
				system("cls");
				addProduct(_productList, size, _categoryList, length);
				break;
			case 3:
				system("cls");
				editProduct(_productList, *size);
				break;
			case 4:
				system("cls");
				delProduct(_productList, size);
				break;
			case 5:
				system("cls");
				searchProductByName(_productList, *size);
				break;
			case 6:
				system("cls");
			...
			case 8:
				system("cls");
				saveProductData(_productList, size);
				break;
			case 9:
				printf("Out Of Interface\n");
				return;
