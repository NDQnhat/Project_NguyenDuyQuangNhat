...
void saveCategoryData(struct category* _category, int* size)
{
	...
	{
		FILE* file = fopen("D:\\PTIT2024B EXCERCISE\\C\\store_management\\category.bin", "rb");
		if (file == NULL)
		{
	...
	{
		FILE* file = fopen("D:\\PTIT2024B EXCERCISE\\C\\store_management\\category.bin", "wb");
		if (file == NULL)
		{
...
void searchProductByName(struct product* _product, int size)
{
	...
	getchar();
	for (int i = 0;i < size;i++) {
		if (isFounded(name, _product[i].productName)) {
			flag = 0;
